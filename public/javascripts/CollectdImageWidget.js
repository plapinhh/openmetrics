...
function flot_onDataReceived(widget, result) {
    ...
    $j('#'+widget.id+" div.content-wrapper").fadeOut(100).html(html).fadeIn();

    $j('#'+widget.id+" div.content-wrapper").css({
        'overflow-x':'hidden'
    });

    $j('#'+widget.id+" div.content-wrapper div.graph").width("100%");
    width = $j('#'+widget.id+" div.content-wrapper div.graph").width() - 20;
    $j('#'+widget.id+" div.content-wrapper div.graph").width(width);
    $j('#'+widget.id+" div.content-wrapper div.graph").height($j('#'+widget.id+" div.content-wrapper").height() - 25);
...
//        showLegendAsSidebar(widget.id);
    ...
    var content_wrapper = $j('#'+widget.id+" div.content-wrapper");
    ...
    if (widget.preferences.show_overview == true) {
        $j('#'+widget.id+" div.content-wrapper div.graph").height($j('#'+widget.id+" div.content-wrapper div.graph").height() - 75);
        $j('#'+widget.id+" div.content-wrapper div.overview").width(width+4);
        $j('#'+widget.id+" div.content-wrapper div.overview").height(75);
        var overview = content_wrapper.children("div.overview");
        var overviewOptions = getOverviewOptions();

        // Draw the overview
        var overview_plot = $j.plot(overview, result['series'], $j.extend(true, {}, overviewOptions, result['options']));
        ...
        overview.bind("plotselected", function (event, ranges) {
            ...
            graph_plot = $j.plot(graph, result['series'], $j.extend(true, {}, graphOptions, result['options']));
        ...
        $j('#'+widget.id+" div.content-wrapper div.overview").remove();
    }
    

    //    if (widget.preferences.show_legend == true) {
    //        $j('#'+widget.id+" div.content-wrapper div.graph").height($j('#'+widget.id+" div.content-wrapper div.graph").height() - 25);
    //        $j('#'+widget.id+" div.content-wrapper div.graph_legend").width(width);
    //        $j('#'+widget.id+" div.content-wrapper div.graph_legend").height(25);
    //        showLegendAsPortlet(content_wrapper);
    //    } else {
    //        $j('#'+widget.id+" div.content-wrapper div.graph_legend").remove();
    //    }

    // Draw the main graph
    var graph_plot = $j.plot(graph, result['series'], $j.extend(true, {}, graphOptions, result['options']));
    customizeGraphCanvas(graph_plot, graph);
    dataVoids = highlightDataVoids(graph_plot, graph);
    if (widget.preferences.show_events) highlightEvents(graph_plot, graph);
    ...
    graph.bind("plotunselected", function(event) {
        ...
        graph_plot = $j.plot(graph, result['series'], $j.extend(true, {}, graphOptions, result['options']));
        if (overview_plot) overview_plot = $j.plot(overview, result['series'], $j.extend(true, {}, overviewOptions, result['options']));
        customizeGraphCanvas(graph_plot, graph);
        dataVoids = highlightDataVoids(graph_plot, graph);
        if (widget.preferences.show_events) highlightEvents(graph_plot, graph);
    });
    ...
    graph.first("canvas").bind("mouseleave", function () {
        $j('#collectd_graph_tooltip').fadeOut(200);
    });
...
function showEventsTooltip(event, pos) {
    if (event) {
        ...
        string += "</div>";
        $j('#collectd_graph_tooltip').html(string).show();
        var width = $j(window).width();
        if (pos.pageX < (width/2)) {
            $j('#collectd_graph_tooltip').css({
                top: (pos.pageY-75),
                left: (pos.pageX+25),
                'border-color': '#fff'
            });
            ...
            width = $j('#collectd_graph_tooltip').width();
            $j('#collectd_graph_tooltip').css({
                top: (pos.pageY-75),
        ...
        $j('<div id="collectd_graph_tooltip" class="collectd_graph_tooltip"><div>').appendTo('#content').hide();
    }
}
...
function showDatavoidsTooltip(dataVoids, pos, item) {
    ...
    var labels = [];
    $j.each(dataVoids, function (label, ranges) {
        $j.each(ranges, function(index, range){
...
function showDatapointTooltip(pos, item) {
    if (item) {
        if ($j('#collectd_graph_tooltip').length != 0) {
...
function highlightDataVoids(graph_plot, graph) {
    ...
    var canvas = $j(graph_canvas).prev('.canvas.custom');

    if (canvas.length == 0) {
        graph_canvas = $j(graph_canvas);
        var width = $j(graph_canvas).width();
        var height = $j(graph_canvas).height();
    ...
    $j.each(series, function(x, serie) {
        var start, finish;
        $j.each(serie.data, function (s, datapoint) {
    ...
    $j(canvas).prependTo(graph_canvas.parent());
    return dataVoids;
}
...
function highlightEvents(graph_plot, graph) {
    ...
    var ctx_width = $j(ctx).width();
    var ctx_height = $j(ctx).height() - offset.top - offset.bottom - 3 - 3 - 6;
    
    var event_height = (Math.floor(ctx_height / events.size()) > 16) ? 16 : Math.floor(ctx_height / events.size());
    ...
    $j.each(events, function(i, event) {
        ...
        if (a.left < offset.left) a.left = offset.left;
        if (b.left > ctx_width) b.left = $j(ctx).width() - offset.right - 3;
        var event_width = Math.max((b.left-a.left - 3 - 4), 16);
        var icon = $j('<img height="'+event_height+'" style="cursor:pointer;" src="/images/noun_icons/red3/16x16/calendar.png" style="vertical-align:middle;" />')
        .click(function(){
            $j(location).attr('href',"/events/"+event.id);
        });
        //            var div = $j('<div class="collectd_graph_event"></div>')
        var div = $j('<div class="collectd_graph_event ui-widget-header ui-corner-all "></div>')
        .append(icon)
        .css({
            left: (a.left),
            top: top
...
function processOptions(metricOptions) {
    if(!$j.isFunction(metricOptions['yaxis']['tickFormatter'])) {
...
function getGraphOptions(id) {
    var legend_container = $j('#'+id + " div.resize-wrapper").find("div.graph_legend_content");
    return {
        shadowSize: 0,
        lines: {
            show: true,
        ...
        legend: {
            ...
            //            container: $j('#'+id + " div.resize-wrapper").find("div.graph_legend_content")
...
function showLegendAsPortlet(content_wrapper) {
    if (content_wrapper.children(".graph_legend").find("span.ui-icon").size() == 0) {
        ...
        .css("cursor", "pointer")
        .click(function() {
            $j(this).children(".ui-icon").toggleClass("ui-icon-minusthick").toggleClass("ui-icon-plusthick").css({
                display: "inline-block"
            });
            $j(this).parents(".graph_legend:first").find(".graph_legend_content").toggle();
        }).click();
    }
}
...
function showLegendAsSidebar(widget){
    ...
    //    var widget = Widgets[id];
    var id = widget.id;
    var width = $j('#'+id + " div.resize-wrapper").width();
    var height = $j('#'+id + " div.resize-wrapper").height();
    

    var legend = $j("<div class='collectd_graph_legend hidden'></div>");
...
//    legend.css("height", widget.sizey - 2);
    ...
    $j('#'+id + " div.resize-wrapper div.collectd_graph_legend").remove();
    $j('#'+id + " div.resize-wrapper").prepend(legend);
    legend.find("div.graph_legend_content").css("height", height - 20);

    var legend_expander = $j("<div class ='graph_legend_expander ui-state-default  ui-corner-right'></div>");
    ...
    .css("cursor", "pointer")
    .click(function() {
        $j(this).parent().find('div.graph_legend_expandable').toggle(200);
        $j(this).parent('div.expanded').animate({'width': '10px'}, 500);
        $j(this).parent('div.hidden').animate({'width': '300px'}, 300);
        $j(this).parent().toggleClass('expanded').toggleClass('hidden');
        return false;
    })
    .click();
}
