...
/* Apply various themes to your site.
   ...
   jQuery('link').themes({themes: ['cupertino', 'smoothness']});
*/
...
function Themes() {
	...
	this._settings = {
		...
		icons: '/images/themes.gif', // Horizontal amalgamation of all theme icons
		iconSize: [23, 20],  // The width and height of the individual icons
		previews: '/images/themes-preview.gif', // Horizontal amalgamation of all theme previews
	...
	this._themes = {  // The definitions of the available themes
		'blitzer': {display: 'Blitzer', icon: 1, preview: 1,
			url: '/stylesheets/themes/blitzer/jquery-ui.css'},
		'cupertino': {display: 'Cupertino', icon: 2, preview: 2,
			url: '/stylesheets/themes/cupertino/jquery-ui.css'},
		'darkhive': {display: 'Dark Hive', icon: 17, preview: 17,
			url: '/stylesheets/themes/dark-hive/jquery-ui.css'},
		'flick': {display: 'Flick', icon: 19, preview: 19,
			url: '/stylesheets/themes/flick/jquery-ui.css'},
		'redmond': {display: 'Redmond', icon: 8, preview: 8,
			url: '/stylesheets/themes/redmond/jquery-ui.css'},
		'southstreet': {display: 'South Street', icon: 10, preview: 10,
			url: '/stylesheets/themes/south-street/jquery-ui.css'},
		'start': {display: 'Start', icon: 11, preview: 11,
			url: '/stylesheets/themes/start/jquery-ui.css'},
                'blacktie': {display: 'Black Tie', icon: 0, preview: 0,
			url: '/stylesheets/themes/black-tie/jquery-ui.css'},
		'sunny': {display: 'Sunny', icon: 23, preview: 23,
			url: '/stylesheets/themes/sunny/jquery-ui.css'},
		'swankypurse': {display: 'Swanky Purse', icon: 12, preview: 12,
			url: '/stylesheets/themes/swanky-purse/jquery-ui.css'},
		'trontastic': {display: 'Trontastic', icon: 13, preview: 13,
			url: '/stylesheets/themes/trontastic/jquery-ui.css'},
		'uidarkness': {display: 'UI Darkess', icon: 14, preview: 14,
			url: '/stylesheets/themes/ui-darkness/jquery-ui.css'},
		'uilightness': {display: 'UI Lightness', icon: 15, preview: 15,
			url: '/stylesheets/themes/ui-lightness/jquery-ui.css'},
		'vader': {display: 'Vader', icon: 16, preview: 16,
			url: '/stylesheets/themes/vader/jquery-ui.css'},
		'smoothness': {display: 'Smoothness', icon: 9, preview: 9,
			url: '/stylesheets/themes/smoothness/jquery-ui.css'},
		'hotsneaks': {display: 'Hot Sneaks', icon: 5, preview: 5,
			url: '/stylesheets/themes/hot-sneaks/jquery-ui.css'},
		'humanity': {display: 'Humanity', icon: 6, preview: 6,
			url: '/stylesheets/themes/humanity/jquery-ui.css'},
		'lefrog': {display: 'Le Frog', icon: 20, preview: 20,
			url: '/stylesheets/themes/le-frog/jquery-ui.css'},
		'mintchoc': {display: 'Mint Choc', icon: 7, preview: 7,
			url: '/stylesheets/themes/mint-choc/jquery-ui.css'},
		'overcast': {display: 'Overcast', icon: 21, preview: 21,
			url: '/stylesheets/themes/overcast/jquery-ui.css'},
		'peppergrinder': {display: 'Pepper Grinder', icon: 22, preview: 22,
			url: '/stylesheets/themes/pepper-grinder/jquery-ui.css'},
		'dotluv': {display: 'Dot Luv', icon: 3, preview: 3,
			url: '/stylesheets/themes/dot-luv/jquery-ui.css'},
		'eggplant': {display: 'Eggplant', icon: 18, preview: 18,
			url: '/stylesheets/themes/eggplant/jquery-ui.css'},
		'excitebike': {display: 'Excite Bike', icon: 4, preview: 4,
			url: '/stylesheets/themes/excite-bike/jquery-ui.css'}
	};
}


jQuery.extend(Themes.prototype, {
	...
	_updateTheme: function(target, settings) {
		settings = extendRemove(jQuery.extend({}, this._settings, this._defaults), settings);
		var themes = settings.themes;
		if (themes.length == 0) {
			jQuery.each(this._themes, function(id) {
		...
		jQuery.each(themes, function(index, id) {
			var theme = allThemes[id];
			if (!theme) {
				return;
			}
			html += '<li class="themes__' + id + (jQuery.themes.currentTheme == id ? ' themes_current' : '') +
				'"><a onclick="jQuery.themes._setTheme(\'' + id + '\',\'' +
				escape(theme.display) + '\',\'' + escape(settings.themeBase + theme.url) + '\')"' +
				(!settings.showPreview ? '' :
				' onmouseover="jQuery.themes._showPreview(this, \'#' + previewId + '\',\'' +
				escape(theme.display) + '\',' + (typeof theme.preview == 'number' ? theme.preview :
				'\'' + theme.preview + '\'') + ',' + settings.previewSize[0] + ')" ' +
				'onmouseout="jQuery.themes._hidePreview(\'#' + previewId + '\')"') + '>';
			if (theme.icon != null) {
	...
	_setTheme: function(id, display, url, loading) {
		if (jQuery('#' + jQuery.themes._linkID).length == 0) {
			jQuery('head').append('<link rel="stylesheet" type="text/css" id="' + jQuery.themes._linkID + '"/>');
		}
		jQuery('#' + jQuery.themes._linkID).attr('href', url);
		jQuery.themes.currentTheme = id;
		jQuery('.' + jQuery.themes.markerClassName + ' li').removeClass('themes_current');
		jQuery('.themes__' + id).addClass('themes_current');
		// Custom callback
		var onSelect = jQuery.themes._defaults.onSelect;
		if (onSelect) {
		...
		if (!loading) {
			...
			var expiryDate = (jQuery.themes._defaults.cookieExpiry ?
				(typeof jQuery.themes._defaults.cookieExpiry == 'number' ?
				addDays(jQuery.themes._defaults.cookieExpiry) : jQuery.themes._defaults.cookieExpiry) : null);
			// Save theme setting as cookie
			document.cookie = COOKIE_NAME + '=' + id +
				(expiryDate ? '; expires=' + expiryDate.toUTCString() : '') +
				(jQuery.themes._defaults.cookiePath ? '; path=' + jQuery.themes._defaults.cookiePath : '') +
				(jQuery.themes._defaults.cookieDomain ? '; domain=' + jQuery.themes._defaults.cookieDomain : '');
		}
		return false;
	},
	...
	_showPreview: function(link, id, display, preview, width) {
		...
		var absParent = null;
		jQuery(link).parents().each(function() {
			if (jQuery(this).css('position') == 'absolute') {
				absParent = jQuery(this);
			}
			return !absParent;
		});
		...
		jQuery(id).children(':not(:first)').remove().end().
...
jQuery.fn.themes = function(options) {
	var otherArgs = Array.prototype.slice.call(arguments, 1);
	return this.each(function() {
		if (typeof options == 'string') {
			jQuery.themes['_' + options + 'Themes'].
				apply(jQuery.themes, [this].concat(otherArgs));
		}
		else {
			jQuery.themes._attachThemes(this, options || {});
		}
	});
};

/* Initialise the themeing functionality. */
jQuery.themes = new Themes(); // singleton instance
