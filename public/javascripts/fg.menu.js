...
jQuery.fn.menuFilament = function(options){
	var caller = this;
	var options = options;
...
//		crumbDefaultText: 'Choose an option:',
	...
	this.create = function(){	
		...
		container.find('#navigation li:has(ul)').attr('aria-haspopup', 'true').find('ul').attr('aria-expanded', 'false');
	...
	this.chooseItem = function(item){
		...
		$j('#menuSelection').text($j(item).text());
	...
	topList.find('a').each(function(){
		...
		if ($j(this).next().is('ul')) {
			$j(this)
				...
				.each(function(){ $j(this).html('<span>' + $j(this).text() + '</span><span class="ui-icon '+options.nextMenuLink+'"></span>'); })
...
Number.prototype.pxToEm = String.prototype.pxToEm = function(settings){
	...
	if (settings.scope == 'body' && $.browser.msie && (parseFloat($j('body').css('font-size')) / getWindowWidth()).toFixed(1) > 0.0) {
