...
function renderWidget(id){
    ...
    var handleDnD = $j('<div></div>');
    handleDnD.addClass('handleDnD');
    handleDnD.css({
        'z-index': zIndex2
    });
    // resize container
    var widgetResizeContainer = $j('<div></div>');
    ...
    var editBar = $j('<div class="widget-toolbar"></div>').hide();
    var moveButton = $j('<img src="/images/noun_icons/'+iconColor+'/24x24/eject.png" />').addClass('widget-toolbar-icon').attr('title', 'Reassign to another dashboard').click(function () {
        getMoveWidgetForm(id);
    });
    var editButton = $j('<img src="/images/noun_icons/'+iconColor+'/24x24/edit.png" />').addClass('widget-toolbar-icon').attr('title', 'Edit').click(function () {
        getEditWidgetForm(id);
    });
    var copyButton = $j('<img src="/images/noun_icons/'+iconColor+'/24x24/paper.png" />').addClass('widget-toolbar-icon').attr('title', 'Copy').click(function () {
        var new_widget = $j.extend(true, {}, widget);
        new_widget.id = 0;
        Widgets[0] = new_widget;
        //            getEditWidgetForm(new_widget);
        getEditWidgetForm(0);
    });
    var deleteButton = $j('<img src="/images/noun_icons/'+iconColor+'/24x24/litter.png" />').addClass('widget-toolbar-icon').attr('title', 'Delete').click(function () {
        deleteWidgets([id]);
    });
    ...
    $j('#content').append(widgetDnDContainer);
    ...
    if (!widget.sizex || !widget.sizey) {
        ...
        widgetContentContainer.children(':first-child').load(function(){
            widget.sizex = $j(this).width();
            widget.sizey = $j(this).height();
    ...
    widgetDnDContainer.children('div.resize-wrapper').resizable({
        ...
        stop: function(event, ui) {
            ...
            var postdata = 'sizex='+$j(this).width()+'&sizey='+$j(this).height() +'&zindex='+maxZindex;
    ...
    widgetDnDContainer.children('div.resize-wrapper').bind("resize", function (event, ui) {
        $j('#'+id+' div.content-wrapper').width($j(this).width());
        $j('#'+id+' div.handleDnD').width($j(this).width());

        $j('#'+id+' div.content-wrapper').height($j(this).height());
        $j('#'+id+' div.handleDnD').height($j(this).height());
    });
    ...
    widgetDnDContainer.draggable({
        drag: function(event, ui) {
            ...
            selected.not(this).each(function() {
                ...
                var el = $j(this), off = el.data("offset");
                el.css({
                    top: off.top + dt,
                    left: off.left + dl
                });
        ...
        start: function(event, ui) {
            ...
            if( !$j(this).hasClass("ui-selected")) {
                ...
                selected = $j(".ui-selected").each(function() {
                    var el = $j(this);
    ...
    if ($j('#editDashboardInput').attr('checked'))
        ...
        $j('div.ui-resizable-handle').hide();
    maxZindex = maxZindex + 3;
...
function deleteWidgets(ids) {
    var dialogContainer = $j('<div></div>').attr('id', 'deleteWidgetsDialog').html('Do you really want to destroy the widget(s)?');
...
function getMoveWidgetForm(id) {
    ...
    var dialogContainer = $j('<div class="ui-helper-clearfix"></div>').attr('id', 'moveWidgetDialog');
        ...
        complete: function(data, textStatus){
            if(textStatus=="success") {
                ...
                select.change(function (){
                    if ($j(this).val() != dashboardId) {
                        var new_dashboard_id = $j(this).val();
...
function placePauseRefreshButton(id){
    ...
    $j('#'+id + " div.pause-button").remove();
    var pauseButton = $j('<div class="pause-button">Refresh is active (interval of '+ refresh +' seconds). Click here to pause refresh. </div>');
    ...
    $j('#'+id + " div.resize-wrapper").prepend(pauseButton);
}
...
function placeStartRefreshButton(id){
    ...
    $j('#'+id + " div.start-button").remove();
    var startButton = $j('<div class="start-button">Refresh is inactive (interval of '+ refresh +' seconds is defined). Click here to start refresh. </div>');
    ...
    $j('#'+id + " div.resize-wrapper").prepend(startButton);
}